...
def load_data():
    ...
    try:
        # Intentamos leer con punto y coma (;) que es tu formato actual
        df = pd.read_csv(file_path, sep=';', encoding='utf-8', on_bad_lines='skip')
        
        # Si pandas lee mal las columnas por el separador, reintentamos con coma
        if len(df.columns) < 2:
            df = pd.read_csv(file_path, sep=',', encoding='utf-8', on_bad_lines='skip')
        
        # LIMPIEZA: Solo nos quedamos con filas que tengan nombre de taller
        df = df.dropna(subset=['NOMBRE DEL TALLER (MAYUSCULAS)'])
        
        # Diccionario de coordenadas de ciudades
        coords = {
            'AMBATO': [-1.2417, -78.6195], 'TUNGURAHUA - AMBATO': [-1.2417, -78.6195],
            'BABAHOYO': [-1.8022, -79.5344], 'DURAN': [-2.1701, -79.8220],
            'EL COCA': [-0.4667, -76.9833], 'GUAYAQUIL': [-2.1894, -79.8891],
        ...
        # Limpiar columna de ciudad y asignar coordenadas
...
if df is not None:
    ...
    with col1:
        ...
        for _, r in df_filtered.iterrows():
            folium.Marker(
                ...
                popup=f"<b>{r['NOMBRE DEL TALLER (MAYUSCULAS)']}</b>",
    ...
    st.markdown("---")
    st.subheader("📋 Listado Detallado de Talleres")
    columnas_tabla = ["NOMBRE DEL TALLER (MAYUSCULAS)", "CIUDAD BASE", "NUMEROS DE CONTACTO", "LINEAS QUE MANEJAN", "COBERTURA INST AA Y LINEA BLANCA"]
    st.dataframe(df_filtered[columnas_tabla], use_container_width=True, hide_index=True)

else:
    st.error("⚠️ Verifica que el archivo se llame 'base_datos_talleres.csv' y esté en la raíz de tu GitHub.")
